...
def edit_review_queue():
    ...
    return render_template('indexes/edit_review_queue.html',
            ...
            next_page=next_page, prev_page=prev_page, sort=sort,
            edits=edits.items, votes=votes)
...
def edit_line(line_id):
    ...
    if form.validate_on_submit():
        if form.line.data != None and len(form.line.data) <= 200:
            ...
            flash("Line updated.")
            return redirect(redirect_url)
    return render_template('forms/line.html',
            title="Edit Line", form=form)
...
def create_tag(tag_request_id):
    ...
    if form.validate_on_submit():
        if form.tag.data != None and form.description.data != None:
            ...
            flash("Tag created.")
            return redirect(redirect_url)
    return render_template('forms/tag.html',
            title="Create Tag", form=form)
