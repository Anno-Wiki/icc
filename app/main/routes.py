...
def search():
    ...
    next_page = url_for('main.search', q=g.search_form.q.data, page=page + 1)\
        if line_total > page * current_app.config['LINES_PER_SEARCH_PAGE']\
        else None
...
def writer_index():
    ...
    sorts = {
            ...
            'translated': url_for('main.writer_index', sort='translated', page=page),
            }
...
def writer_annotations(writer_url):
    ...
    next_page = url_for('main.writer_annotations', writer_url=writer.url, sort=sort,
            page=annotations.next_num) if annotations.has_next else None
    prev_page = url_for('main.writer_annotations', writer_url=writer.url, sort=sort,
...
def text_index():
    ...
    sorts = {
            ...
            'annotations': url_for('main.text_index', sort='annotations', page=page),
    }
...
def text_annotations(text_url):
    ...
    next_page = url_for('main.text_annotations', text_url=text.url, sort=sort,
            page=annotations.next_num) if annotations.has_next else None
    ...
    if sort == 'newest':
        ...
        annotations = edition.annotations.join(Edit,
                ...
                .order_by(Edit.last_line_num.asc()).paginate(page,
                        current_app.config['ANNOTATIONS_PER_PAGE'], False)
    else:
...
@main.route('/text/<text_url>/edition/<edition_num>/line/<line_num>/annotations')
...
def tag(tag):
    ...
    sorts = {
            ...
            'modified': url_for('main.tag', tag=tag.tag, page=page, sort='modified'),
            }

    next_page = url_for('main.tag', tag=tag.tag, page=annotations.next_num,
            sort=sort) if annotations.has_next else None
    prev_page = url_for('main.tag', tag=tag.tag, page=annotations.prev_num,
...
@main.route('/annotate/<text_url>/<first_line>/<last_line>',
        methods=['GET', 'POST'], defaults={'edition_num':None})
@main.route('/annotate/<text_url>/edition/<edition_num>/<first_line>/<last_line>',
        methods=['GET', 'POST'])
@login_required
...
def upvote(annotation_id):
    ...
    if current_user == annotation.annotator:
        ...
        vote = current_user.voteballots.filter(Vote.annotation==annotation).first()
...
def edit_history(annotation_id):
    ...
    next_page = url_for('main.edit_review_queue', page=edits.next_num, sort=sort)\
            if edits.has_next else None
    prev_page = url_for('main.edit_review_queue', page=edits.prev_num, sort=sort)\
